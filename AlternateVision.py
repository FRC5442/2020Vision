...
#5442 LIDAR VISION V2, Surrogate File, LAST EDIT 1/28/2020
...
def findTape(contours, image, centerX, centerY):
    ...
    if len(contours) >= 2:
        ...
        for i in range(len(biggestCnts) - 1):
            ...
            '''
			5442 EDIT START 
            ImageX = math.floor((cx1 + cx2) / 2)
            ImageY = math.floor((cy1 + cy2) / 2)
			5442 EDIT END  
			'''
    ...
    if (len(targets) > 0):
		...
		5442 EDIT START 
		'''
        networkTable.putNumber("OffsetX", (((cx1 + cx2)/2) - centerX))
        networkTable.putNumber("OffsetY", (cy2 - centerY))
        '''
		5442 EDIT END  
		'''
    else:
